...
def list_todo():
    with get_session() as db:
        # We select Todo and join User to get the owner's name
        statement = select(Todo)
        results = db.exec(statement).all()
        
        print(f"{'ID':<4} | {'Text':<20} | {'User':<10} | {'Done'}")
        print("-" * 50)
        
        for todo in results:
            status = "Yes" if todo.done else "No"
            # Accessing username via the relationship defined in models.py
            print(f"{todo.id:<4} | {todo.text:<20} | {todo.user.username:<10} | {status}")
...
def delete_todo(todo_id: int):
    with get_session() as db:
        ...
        db.delete(todo)
        db.commit()
        print(f"Success: Task {todo_id} deleted.")
...
def complete_all(username: str):
    with get_session() as db:
        ...
        db.commit()
        print(f"Success: All tasks for {username} marked as complete!")
